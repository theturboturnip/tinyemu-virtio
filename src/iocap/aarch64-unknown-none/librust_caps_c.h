...
* Generated by cbindgen from the rust_caps_c Rust crate version 0.3.0.
*/
...
/**
 * Initialize a Cap2024_11 capability from scratch allowing access to the full 64-bit address range (base = 0, len = 1<<64), given the permissions (Read|Write|Both), and the secret id.
 ...
 */
CCapResult ccap_init_almighty(struct CCap2024_11 *cap,
                              const CCapU128 *secret,
                              uint32_t secret_id,
                              CCapPerms perms);

/**
 * Initialize a Cap2024_11 capability from scratch, given the contiguous memory range it grants access to, the permissions (Read|Write|Both), and the secret_id.
 ...
 * Does not use caveats.
 */
CCapResult ccap_init_exact(struct CCap2024_11 *cap,
                           const CCapU128 *secret,
                           uint64_t base,
                           uint64_t len,
                           uint32_t secret_id,
 ...
 * Calculates the capability signature given the packed data and the secret.
 */
CCapResult ccap_init_cavs_exact(struct CCap2024_11 *cap,
                                const CCapU128 *secret,
                                uint64_t base,
                                uint64_t len,
                                uint32_t secret_id,
 ...
 * Does not use caveats.
 */
CCapResult ccap_init_inexact(struct CCap2024_11 *cap,
                             const CCapU128 *secret,
                             uint64_t base,
                             uint64_t len,
                             uint32_t secret_id,
 ...
 */
CCapResult ccap_check_signature(const struct CCap2024_11 *cap,
                                const CCapU128 *secret);
 ...
 */
CCapResult ccap_read_range(const struct CCap2024_11 *cap,
                           uint64_t *base,
                           uint64_t *len,
                           bool *len_64);
 ...
 * Doesn't check the capability signature.
 */
CCapResult ccap_read_perms(const struct CCap2024_11 *cap, CCapPerms *perms);
 ...
 * Doesn't check the capability signature.
 */
CCapResult ccap_read_secret_id(const struct CCap2024_11 *cap, uint32_t *secret_id);
